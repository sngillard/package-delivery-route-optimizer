...
class HashTable:
    def __init__(self, size = 41): # Prime number size of buckets helps spread out key-value pairs and reduce collisions.
        self.size = size
    ...
    def _get_hash(self, key):
            # Create a simple hash function to compute the array index for a given package_id 
            hash_value = 0
            # Loop through each character in the key
            for char in str(key):
                 hash_value += ord(char) # ord is the ASCII numerical value of a char in Python
            return hash_value % self.size # Use modulus to get an index value within the table size
    
    def add(self, key, value):
        ''' 
        Insertion function to add the key-value pair to the HashTable.
        ...
        if self.count / self.size >= 0.7: 
              self._resize() # To improve scalability and reduce collisions as the dataset grows, the hash table will automatically double in size and rehash elements once it is 70% full (load factor exceeds 0.7)
    ...
    # Lookup function that takes the package ID as input and returns the delivery address, deadline, city, and zip code, plus package weight and delivery status including delivery time
    def get(self, key):
    ...
    def _resize(self):
        ...
        for bucket in former_map:
            if bucket is not None:
                for pair in bucket:
                    self.add(pair[0], pair[1]) # Re-add all key-value pairs to the re-sized table
